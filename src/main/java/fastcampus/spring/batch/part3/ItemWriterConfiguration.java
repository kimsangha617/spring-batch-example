...
//                .next(this.jdbcBatchItemWriterStep())
    ...
    private ItemWriter<Person> jdbcBatchItemWriter() {
        JdbcBatchItemWriter<Person> itemWriter = new JdbcBatchItemWriterBuilder<Person>()
                ...
                .itemSqlParameterSourceProvider(new BeanPropertyItemSqlParameterSourceProvider<>()) //Person 클래스를 파라미터로 자동으로 생성하는
                .sql("insert into person(name,age,addr) values(:name, :age, :addr)")
                .build();
        itemWriter.afterPropertiesSet();
        return itemWriter;
    }
    ...
    private ItemWriter<Person> csvFileItemWriter() throws Exception {
        ...
        fieldExtractor.setNames(new String[] {"id", "name", "age", "addr"});
